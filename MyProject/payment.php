               ...
               echo "<div class='bike-image-wrapper'>
                    ...
                    <ul>";
                       echo "<p id='stock'>In stock</p>
                        ...
                        if($bikeenginevalue >= 350){
                            ...
                            $totalamt = 90/100 * $advanceamt;
                            $baseprice = $bikeprice - $advanceamt;
                            $basepayamt = 90/100 * $baseprice;
                        }else{
                            $advanceamt = $row2['lessthan350cc']/100 * $bikeprice;
                            $totalamt = 92/100 * $advanceamt;
                            $baseprice = $bikeprice - $advanceamt;
                            $basepayamt = 92/100 * $baseprice;
                        }
                       echo "<li id='item'><span id='price-price'>Price:</span>
                        <span id='price'><i class='fas fa-rupee-sign' aria-hidden='true'><strong>".number_format($totalamt + $basepayamt,0)."</strong></i>";
                        }
                    }
                        ?></span></li>
                    </ul>
                ...
                <div class='payment-methods-container'>
                    ...
                    <p id='payment-methods-text'><strong>Please select any one method of payment</strong></p>
                        <form id='form1' action='includes/payment.inc.php' method='post'>
                            ...
                            <input type='radio' name='radio' id='cash' value='cash'>CASH<br><hr>
                        ...
                        $sqlcompany="select * from companydetails";
                            ...
                            -Bank Branch:<?php echo $rows1['companybranchname'];?><br></strong>
                            -Your booking will be confirmed once we get the amount through cheque,NEFT or cash.<br>
                            -The Bike will be added into your <strong>Cart</strong> where you can proceed to make the final payment and the amount has to be paid before a week of your bikes delivery.<br>
                        -     Once the booking is confirmed and in any case of cancellation of the booking required please send an email to <?php $sqlcompany="select * from companydetails";
                        $resultcompany = mysqli_query($conn,$sqlcompany);
                        $rows1 = mysqli_fetch_assoc($resultcompany);
                        echo $rows1['companyemail'];?>,but the advance amount will not be refunded due to the companies policies.
